                  ...
                  <Box sx={{ mt: 2, width: '100%', display: 'flex', flexDirection: 'column', gap: 1 }}>
                    {student.tesserini.map((tess, tessIdx) => (
                      <Box key={tessIdx} sx={{ mb: 1 }}>
                        ...
                        <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', maxWidth: 420 }}>
                          {tess.lessons && tess.lessons.map((lesson, lessonIdx) => (
                              ...
                              onClick={async () => {
                                if (!lesson.isUsed) {
                                  try {
                                    ...
                                    axios.get(`${apiUrl}/api/students`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }).then(res => setStudents(res.data));
                            ...
                            >
                              {lesson.isUsed ? <span style={{ color: 'red', fontWeight: 900 }}>X</span> : lessonIdx + 1}
                            </Box>
                          ))}
                        </Box>
                      </Box>
